...
### 1.1.1 P1 补充制度：快车道、横向咨询、中央队列 SLA

`P1-8` 在不破坏主状态机的前提下，为高吞吐任务补了三项制度化机制：

- `lane = standard | fast`
  快车道只改变处理时效，不新增主状态，也不绕过门下省审议。`fast` 任务在 `Menxia -> Assigned` 之后，如果已经明确唯一执行部门，则允许按合法迁移自动执行 `Assigned -> Next`，从而减少尚书省纯转运等待。
- `consultLog`
  横向咨询是“有约束的求助”，只能由当前状态负责人发起，目标仍受独立咨询权限矩阵约束。咨询会留下审计记录和活动流，但**不会**改变任务主状态、负责人或所有权。
- `queueMetrics + SLA`
  门下省与尚书省被视为中央队列节点。系统对 `Menxia / Assigned / Review` 计算等待量、超时量、快车道占比与最老等待时长，并将推荐 SLA 注入调度器，用于停滞扫描与拥塞观测。

因此，制度契约在 `P1` 之后变为：

- 所有任务仍必须遵守单一主状态机，不能用“快车道”绕开准奏、执行、审查等主链路。
- 横向咨询只能补充信息，不能偷偷完成派发、审批或越权改态。
- 中央节点的吞吐优化必须通过显式 `lane`、咨询审计和队列监控完成，而不是靠绕开制度。
...
```json
{
  ...
  "lane": "fast",                    // 通道：standard|fast（只影响时效，不改变主状态机）
  ...
  "_scheduler": {
    "enabled": true,
    "stallThresholdSec": 300,         // 中央节点默认继承 lane 对应 SLA
    "maxRetry": 1,                    // 自动重试最多1次
    "retryCount": 0,
