...
<p align="center">
  <sub>12 个 AI Agent（11 个业务角色 + 1 个兼容角色）组成三省六部：司礼监分办、中书省规划、门下省审核封驳、尚书省派发、六部+吏部并行执行。<br>比 CrewAI 多一层<b>制度性审核</b>，比 AutoGen 多一个<b>实时看板</b>。</sub>
</p>
...
<p align="center">
  ...
  <br>
  <sub>飞书下旨 → 司礼监分办 → 中书省规划 → 门下省审议 → 六部并行执行 → 奏折回报（30 秒）</sub>
</p>
</details>
...
```
你 (皇上) → 司礼监 (承旨分办) → 中书省 (规划) → 门下省 (审议) → 尚书省 (派发) → 六部 (执行) → 回奏
```
...
- 支持多轮推进 · 总结结论 · 讨论记录落盘保存，可跨重启恢复/复盘
...
安装脚本自动完成：
- ✅ 创建全量 Agent Workspace（含司礼监/吏部/早朝，兼容历史 main）
...
| 部门 | Agent ID | 职责 | 擅长领域 |
|------|----------|------|---------|
| 🧾 **司礼监** | `sili` | 承旨分办、回奏传达 | 闲聊识别、旨意提炼、回奏转达 |
...
| From ↓ \ To → | 司礼监 | 中书 | 门下 | 尚书 | 户 | 礼 | 兵 | 刑 | 工 | 吏 |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **司礼监** | — | ✅ | | | | | | | | |
| **中书省** | ✅ | — | ✅ | ✅ | | | | | | |
| **门下省** | | ✅ | — | ✅ | | | | | | |
...
```
皇上 → 司礼监分办 → 中书规划 → 门下审议 → 已派发 → 执行中 → 待审查 → ✅ 已完成
                      ↑          │                              │
                      └──── 封驳 ─┘                    阻塞 Blocked
```

> ⚡ **状态转换受保护**：`kanban_update.py` 内置 `_VALID_TRANSITIONS` 状态机校验，
> 非法跳转（如 Doing→Sili）会被拒绝并记录日志，确保流程不可绕过。
...
<details>
<summary><b>❌ 任务总超时 / 下属完成了但无法传回司礼监</b></summary>

**症状**：六部或尚书省已完成任务，但司礼监收不到回报，最终超时。
...
3. **常见原因**：
   - Agent ID 不匹配（已在 v1.2 修复：`main` → `sili`）
...
### Phase 1 — 核心架构 ✅
- [x] 十二部制 Agent 架构（司礼监 + 三省 + 七部 + 早朝官）+ 权限矩阵
